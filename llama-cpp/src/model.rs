...
impl LlamaModel {
    ...
    pub fn n_head_kv(&self) -> u32 {
        // It's never possible for this to panic because while the API interface is defined as an int32_t,
        // the field it's accessing is a uint32_t.
        u32::try_from(unsafe { llama_cpp_sys::llama_model_n_head_kv(self.model.as_ptr()) })
            .unwrap()
    }
    ...
    pub fn apply_chat_template_with_tools_oaicompat(
        ...
        let chat: Vec<llama_cpp_sys::llama_chat_message> = messages
            .iter()
            .map(|c| llama_cpp_sys::llama_chat_message {
                role: c.role.as_ptr(),
                content: c.content.as_ptr(),
        ...
        let mut raw_result = llama_cpp_sys::llama_rs_chat_template_result {
            prompt: ptr::null_mut(),
        ...
        let rc = unsafe {
            llama_cpp_sys::llama_rs_apply_chat_template_with_tools_oaicompat(
                self.model.as_ptr(),
                tmpl.0.as_ptr(),
                chat.as_ptr(),
                chat.len(),
        ...
        unsafe { llama_cpp_sys::llama_rs_chat_template_result_free(&mut raw_result) };
        result
    }
    ...
    pub fn apply_chat_template_oaicompat(
        ...
        let ffi_params = llama_cpp_sys::llama_rs_chat_template_oaicompat_params {
        ...
        let rc = unsafe {
            llama_cpp_sys::llama_rs_apply_chat_template_oaicompat(
                self.model.as_ptr(),
                tmpl.0.as_ptr(),
                &ffi_params,
                &mut raw_result,
...
impl ChatTemplateResult {
    ...
    pub fn parse_response_oaicompat(
        ...
        let rc = unsafe {
            llama_cpp_sys::llama_rs_chat_parse_to_oaicompat(
                text_cstr.as_ptr(),
                is_partial,
                self.chat_format,
    ...
    pub fn streaming_state_oaicompat(&self) -> Result<ChatParseStateOaicompat, ChatParseError> {
        ...
        let state = unsafe {
            llama_cpp_sys::llama_rs_chat_parse_state_init_oaicompat(
                self.chat_format,
                self.parse_tool_calls,
                parser_cstr
                    .as_ref()
