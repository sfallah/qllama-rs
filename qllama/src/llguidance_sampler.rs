...
fn build_tok_env(model: &LlamaModel) -> Arc<ApproximateTokEnv> {
    ...
    let tok_eos = {
        let eot = unsafe { qllama_sys::llama_vocab_eot(model.vocab_ptr()) };
        if eot == -1 {
...
unsafe extern "C" fn llg_name(
    _smpl: *const qllama_sys::llama_sampler,
) -> *const std::os::raw::c_char {
    c"llguidance".as_ptr()
}

unsafe extern "C" fn llg_accept(
    smpl: *mut qllama_sys::llama_sampler,
    token: qllama_sys::llama_token,
) {
...
unsafe extern "C" fn llg_apply(
    smpl: *mut qllama_sys::llama_sampler,
    cur_p: *mut qllama_sys::llama_token_data_array,
) {
    let ctx = unsafe { &mut *(*smpl).ctx.cast::<LlgContext>() };
...
unsafe extern "C" fn llg_reset(smpl: *mut qllama_sys::llama_sampler) {
    let ctx = unsafe { &mut *(*smpl).ctx.cast::<LlgContext>() };
    let _ = ctx.matcher.reset();
}

unsafe extern "C" fn llg_clone(
    smpl: *const qllama_sys::llama_sampler,
) -> *mut qllama_sys::llama_sampler {
...
unsafe extern "C" fn llg_free(smpl: *mut qllama_sys::llama_sampler) {
...
static mut LLG_SAMPLER_I: qllama_sys::llama_sampler_i = qllama_sys::llama_sampler_i {
    name: Some(llg_name),
