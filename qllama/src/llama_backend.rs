...
impl LlamaBackend {
    ...
    pub fn init() -> crate::Result<LlamaBackend> {
        Self::mark_init()?;
        unsafe { qllama_sys::llama_backend_init() }
        Ok(LlamaBackend {})
    }
    ...
    ///# use qllama::llama_backend::LlamaBackend;
    ///# use std::error::Error;
    ///# use qllama::llama_backend::NumaStrategy;
    ///
    ///# fn main() -> Result<(), Box<dyn Error>> {
    ///
    ...
    pub fn init_numa(strategy: NumaStrategy) -> crate::Result<LlamaBackend> {
        Self::mark_init()?;
        unsafe {
            qllama_sys::llama_numa_init(qllama_sys::ggml_numa_strategy::from(strategy));
        }
        Ok(LlamaBackend {})
    }
    ...
    pub fn void_logs(&mut self) {
        ...
        unsafe {
            qllama_sys::llama_log_set(Some(void_log), std::ptr::null_mut());
        }
    }
}
...
pub struct InvalidNumaStrategy(
    ...
    pub qllama_sys::ggml_numa_strategy,
);

impl TryFrom<qllama_sys::ggml_numa_strategy> for NumaStrategy {
    type Error = InvalidNumaStrategy;

    fn try_from(value: qllama_sys::ggml_numa_strategy) -> Result<Self, Self::Error> {
        match value {
            qllama_sys::GGML_NUMA_STRATEGY_DISABLED => Ok(Self::DISABLED),
            qllama_sys::GGML_NUMA_STRATEGY_DISTRIBUTE => Ok(Self::DISTRIBUTE),
            qllama_sys::GGML_NUMA_STRATEGY_ISOLATE => Ok(Self::ISOLATE),
            qllama_sys::GGML_NUMA_STRATEGY_NUMACTL => Ok(Self::NUMACTL),
            qllama_sys::GGML_NUMA_STRATEGY_MIRROR => Ok(Self::MIRROR),
            qllama_sys::GGML_NUMA_STRATEGY_COUNT => Ok(Self::COUNT),
            value => Err(InvalidNumaStrategy(value)),
        }
    }
}

impl From<NumaStrategy> for qllama_sys::ggml_numa_strategy {
    fn from(value: NumaStrategy) -> Self {
        match value {
            NumaStrategy::DISABLED => qllama_sys::GGML_NUMA_STRATEGY_DISABLED,
            NumaStrategy::DISTRIBUTE => qllama_sys::GGML_NUMA_STRATEGY_DISTRIBUTE,
            NumaStrategy::ISOLATE => qllama_sys::GGML_NUMA_STRATEGY_ISOLATE,
            NumaStrategy::NUMACTL => qllama_sys::GGML_NUMA_STRATEGY_NUMACTL,
            NumaStrategy::MIRROR => qllama_sys::GGML_NUMA_STRATEGY_MIRROR,
            NumaStrategy::COUNT => qllama_sys::GGML_NUMA_STRATEGY_COUNT,
        }
    }
}
...
impl Drop for LlamaBackend {
    fn drop(&mut self) {
        ...
        unsafe { qllama_sys::llama_backend_free() }
    }
}
...
mod tests {
    ...
    fn numa_from_and_to() {
        ...
        for numa in &numas {
            let from = qllama_sys::ggml_numa_strategy::from(*numa);
