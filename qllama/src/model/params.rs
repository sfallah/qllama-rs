...
const LLAMA_SPLIT_MODE_NONE: i8 = qllama_sys::LLAMA_SPLIT_MODE_NONE as i8;
#[allow(clippy::cast_possible_wrap)]
#[allow(clippy::cast_possible_truncation)]
const LLAMA_SPLIT_MODE_LAYER: i8 = qllama_sys::LLAMA_SPLIT_MODE_LAYER as i8;
#[allow(clippy::cast_possible_wrap)]
#[allow(clippy::cast_possible_truncation)]
const LLAMA_SPLIT_MODE_ROW: i8 = qllama_sys::LLAMA_SPLIT_MODE_ROW as i8;
#[allow(clippy::cast_possible_wrap)]
#[allow(clippy::cast_possible_truncation)]
const LLAMA_SPLIT_MODE_TENSOR: i8 = qllama_sys::LLAMA_SPLIT_MODE_TENSOR as i8;
...
fn load_mode_from_flags(use_mmap: bool, use_mlock: bool) -> qllama_sys::llama_load_mode {
    match (use_mmap, use_mlock) {
        (false, false) => qllama_sys::LLAMA_LOAD_MODE_NONE,
        (true, false) => qllama_sys::LLAMA_LOAD_MODE_MMAP,
        (false, true) => qllama_sys::LLAMA_LOAD_MODE_MLOCK,
        (true, true) => qllama_sys::LLAMA_LOAD_MODE_MMAP_MLOCK,
    }
}
...
pub struct LlamaModelParams {
    pub(crate) params: qllama_sys::llama_model_params,
    kv_overrides: Vec<qllama_sys::llama_model_kv_override>,
    buft_overrides: Vec<qllama_sys::llama_model_tensor_buft_override>,
    devices: Pin<Box<[qllama_sys::ggml_backend_dev_t; LLAMA_CPP_MAX_DEVICES]>>,
}
...
impl LlamaModelParams {
    ...
    pub fn append_kv_override(
        ...
        self.kv_overrides
            .push(qllama_sys::llama_model_kv_override {
                key: [0; 128],
                tag: 0,
                __bindgen_anon_1: qllama_sys::llama_model_kv_override__bindgen_ty_1 {
                    val_i64: 0,
                },
            });
...
impl LlamaModelParams {
    ...
    pub fn add_cpu_buft_override(mut self: Pin<&mut Self>, key: &CStr) {
        ...
        buft_override.buft = unsafe { qllama_sys::ggml_backend_cpu_buffer_type() };
        ...
        // push the next one to ensure we maintain the iterator invariant of ending with a 0
        self.buft_overrides
            .push(qllama_sys::llama_model_tensor_buft_override {
...
impl LlamaModelParams {
    ...
    pub fn use_mmap(&self) -> bool {
        matches!(
            ...
            qllama_sys::LLAMA_LOAD_MODE_AUTO
                | qllama_sys::LLAMA_LOAD_MODE_MMAP
                | qllama_sys::LLAMA_LOAD_MODE_MMAP_MLOCK
        )
    }
    ...
    pub fn use_mlock(&self) -> bool {
        matches!(
            self.params.load_mode,
            qllama_sys::LLAMA_LOAD_MODE_MLOCK | qllama_sys::LLAMA_LOAD_MODE_MMAP_MLOCK
        )
    }
    ...
    pub fn devices(&self) -> Vec<usize> {
        ...
        for i in 0..unsafe { qllama_sys::ggml_backend_dev_count() } {
            let dev = unsafe { qllama_sys::ggml_backend_dev_get(i) };
    ...
    pub fn with_devices(mut self, devices: &[usize]) -> Result<Self, LlamaCppError> {
        ...
        for (i, &dev) in devices.iter().enumerate() {
            if dev >= unsafe { qllama_sys::ggml_backend_dev_count() } {
                return Err(LlamaCppError::BackendDeviceNotFound(dev));
            }
            let backend_dev = unsafe { qllama_sys::ggml_backend_dev_get(dev) };
...
/// ```
/// # use qllama::model::params::LlamaModelParams;
/// use qllama::model::params::LlamaSplitMode;
/// let params = LlamaModelParams::default();
...
impl Default for LlamaModelParams {
    fn default() -> Self {
        let default_params = unsafe { qllama_sys::llama_model_default_params() };
        LlamaModelParams {
...
mod tests {
    ...
    fn tensor_split_mode_round_trips() {
        assert_eq!(
            LlamaSplitMode::try_from(qllama_sys::LLAMA_SPLIT_MODE_TENSOR),
            Ok(LlamaSplitMode::Tensor)
        );
        assert_eq!(
            u32::from(LlamaSplitMode::Tensor),
            qllama_sys::LLAMA_SPLIT_MODE_TENSOR as u32
        );
        assert_eq!(
            i32::from(LlamaSplitMode::Tensor),
            qllama_sys::LLAMA_SPLIT_MODE_TENSOR as i32
        );
    }
}
