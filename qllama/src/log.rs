...
fn meta_for_level(
    ...
    match level {
        qllama_sys::GGML_LOG_LEVEL_DEBUG => (&DEBUG_META, &DEBUG_FIELDS),
        qllama_sys::GGML_LOG_LEVEL_INFO => (&INFO_META, &INFO_FIELDS),
        qllama_sys::GGML_LOG_LEVEL_WARN => (&WARN_META, &WARN_FIELDS),
        qllama_sys::GGML_LOG_LEVEL_ERROR => (&ERROR_META, &ERROR_FIELDS),
        _ => {
...
pub(super) struct State {
    ...
    module: Module,
    buffered: std::sync::Mutex<Option<(qllama_sys::ggml_log_level, String)>>,
...
impl State {
    ...
    fn generate_log(target: Module, level: qllama_sys::ggml_log_level, text: &str) {
    ...
    pub(super) fn buffer_non_cont(&self, level: qllama_sys::ggml_log_level, text: &str) {
        debug_assert!(!text.ends_with('\n'));
        debug_assert_ne!(level, qllama_sys::GGML_LOG_LEVEL_CONT);
    ...
    pub(super) fn emit_non_cont_line(&self, level: qllama_sys::ggml_log_level, text: &str) {
        debug_assert!(text.ends_with('\n'));
        debug_assert_ne!(level, qllama_sys::GGML_LOG_LEVEL_CONT);
        ...
        match level {
            qllama_sys::GGML_LOG_LEVEL_NONE => {
                // TODO: Support logging this to stdout directly via options?
                tracing::info!(no_log_level = true, text);
            }
            qllama_sys::GGML_LOG_LEVEL_DEBUG
            | qllama_sys::GGML_LOG_LEVEL_INFO
            | qllama_sys::GGML_LOG_LEVEL_WARN
            | qllama_sys::GGML_LOG_LEVEL_ERROR => Self::generate_log(self.module, level, text),
            qllama_sys::GGML_LOG_LEVEL_CONT => unreachable!(),
            _ => {
                tracing::warn!(
                    level = level,
                    text = text,
    ...
    pub(super) fn is_enabled_for_level(&self, level: qllama_sys::ggml_log_level) -> bool {
        // CONT logs do not need to check if they are enabled.
        let level = if level == qllama_sys::GGML_LOG_LEVEL_CONT {
            self.previous_level
...
mod tests {
    ...
    fn cont_disabled_log() {
        ...
        logs_to_trace(
            qllama_sys::GGML_LOG_LEVEL_CONT,
            c"world\n".as_ptr(),
            log_ptr,
        );
        ...
        logs_to_trace(qllama_sys::GGML_LOG_LEVEL_CONT, c"\n".as_ptr(), log_ptr);
    }
