...
pub struct LlamaModel {
    pub(crate) model: NonNull<qllama_sys::llama_model>,
}
...
pub struct LlamaLoraAdapter {
    pub(crate) lora_adapter: NonNull<qllama_sys::llama_adapter_lora>,
}
...
impl LlamaModel {
    pub(crate) fn vocab_ptr(&self) -> *const qllama_sys::llama_vocab {
        unsafe { qllama_sys::llama_model_get_vocab(self.model.as_ptr()) }
    }
    ...
    pub fn n_ctx_train(&self) -> u32 {
        let n_ctx_train = unsafe { qllama_sys::llama_n_ctx_train(self.model.as_ptr()) };
    ...
    pub fn token_bos(&self) -> LlamaToken {
        let token = unsafe { qllama_sys::llama_token_bos(self.vocab_ptr()) };
        LlamaToken(token)
    }
    ...
    pub fn token_eos(&self) -> LlamaToken {
        let token = unsafe { qllama_sys::llama_token_eos(self.vocab_ptr()) };
        LlamaToken(token)
    }
    ...
    pub fn token_nl(&self) -> LlamaToken {
        let token = unsafe { qllama_sys::llama_token_nl(self.vocab_ptr()) };
        LlamaToken(token)
    }
    ...
    pub fn is_eog_token(&self, token: LlamaToken) -> bool {
        unsafe { qllama_sys::llama_token_is_eog(self.vocab_ptr(), token.0) }
    }
    ...
    pub fn decode_start_token(&self) -> LlamaToken {
        let token =
            unsafe { qllama_sys::llama_model_decoder_start_token(self.model.as_ptr()) };
        LlamaToken(token)
    }
    ...
    pub fn token_sep(&self) -> LlamaToken {
        let token = unsafe { qllama_sys::llama_vocab_sep(self.vocab_ptr()) };
        LlamaToken(token)
    }
    ...
    /// let backend = qllama::llama_backend::LlamaBackend::init()?;
    ...
    pub fn str_to_token(
        ...
        let size = unsafe {
            qllama_sys::llama_tokenize(
                self.vocab_ptr(),
                c_string.as_ptr(),
                c_int::try_from(c_string.as_bytes().len())?,
                buffer.as_mut_ptr().cast::<qllama_sys::llama_token>(),
                buffer_capacity,
                add_bos,
                true,
            )
        ...
        // Safety: `size` < `capacity` and qllama has initialized elements up to `size`
    ...
    pub fn token_attr(&self, LlamaToken(id): LlamaToken) -> LlamaTokenAttrs {
        let token_type = unsafe { qllama_sys::llama_token_get_attr(self.vocab_ptr(), id) };
    ...
    /// # Panics
    ///
    /// - if the returned size from qllama does not fit into a [`usize`]. (this should never happen)
    pub fn token_to_piece(
        ...
        let bytes = match self.token_to_piece_bytes(token, 8, special, lstrip) {
            ...
            // https://github.com/abetlen/qllama-python/blob/c37132bac860fcc333255c36313f89c4f49d4c8d/qllama/qllama.py#L3461
            Err(TokenToStringError::InsufficientBufferSpace(i)) => self.token_to_piece_bytes(
                token,
                (-i).try_into().expect("Error buffer size is positive"),
                special,
    ...
    pub fn token_to_piece_bytes(
        ...
        let size = unsafe {
            qllama_sys::llama_token_to_piece(
                self.vocab_ptr(),
                token.0,
                buf,
                len,
    ...
    /// - if the token type is unknown
    /// - the resultant token is larger than `buffer_size`.
    /// - the string returend by qllama is not valid utf8.
    ///
    /// # Panics
    ///
    /// - if `buffer_size` does not fit into a [`c_int`].
    ...
    pub fn n_vocab(&self) -> i32 {
        unsafe { qllama_sys::llama_n_vocab(self.vocab_ptr()) }
    }
    ...
    /// # Panics
    ///
    /// If qllama emits a vocab type that is not known to this library.
    #[must_use]
    pub fn vocab_type(&self) -> VocabType {
        // qllama_sys::llama_model_get_vocab
        let vocab_type = unsafe { qllama_sys::llama_vocab_type(self.vocab_ptr()) };
    ...
    pub fn n_params(&self) -> u64 {
        unsafe { qllama_sys::llama_model_n_params(self.model.as_ptr()) }
    }

    /// Returns whether the model is a recurrent network (Mamba, RWKV, etc)
    pub fn is_recurrent(&self) -> bool {
        unsafe { qllama_sys::llama_model_is_recurrent(self.model.as_ptr()) }
    }
    ...
    pub fn is_hybrid(&self) -> bool {
        unsafe { qllama_sys::llama_model_is_hybrid(self.model.as_ptr()) }
    }
    ...
    pub fn n_layer(&self) -> u32 {
        ...
        u32::try_from(unsafe { qllama_sys::llama_model_n_layer(self.model.as_ptr()) }).unwrap()
    }
    ...
    pub fn n_head(&self) -> u32 {
        // It's never possible for this to panic because while the API interface is defined as an int32_t,
        // the field it's accessing is a uint32_t.
        u32::try_from(unsafe { qllama_sys::llama_model_n_head(self.model.as_ptr()) }).unwrap()
    }
    ...
    pub fn n_head_kv(&self) -> u32 {
        // It's never possible for this to panic because while the API interface is defined as an int32_t,
        // the field it's accessing is a uint32_t.
        u32::try_from(unsafe { qllama_sys::llama_model_n_head_kv(self.model.as_ptr()) })
            .unwrap()
    }
    ...
    pub fn meta_val_str(&self, key: &str) -> Result<String, MetaValError> {
        ...
        extract_meta_string(
            |buf_ptr, buf_len| unsafe {
                qllama_sys::llama_model_meta_val_str(
                    self.model.as_ptr(),
                    key_ptr,
                    buf_ptr,
                    buf_len,
    ...
    pub fn meta_count(&self) -> i32 {
        unsafe { qllama_sys::llama_model_meta_count(self.model.as_ptr()) }
    }
    ...
    pub fn meta_key_by_index(&self, index: i32) -> Result<String, MetaValError> {
        extract_meta_string(
            |buf_ptr, buf_len| unsafe {
                qllama_sys::llama_model_meta_key_by_index(
                    self.model.as_ptr(),
                    index,
                    buf_ptr,
                    buf_len,
    ...
    pub fn meta_val_str_by_index(&self, index: i32) -> Result<String, MetaValError> {
        extract_meta_string(
            |buf_ptr, buf_len| unsafe {
                qllama_sys::llama_model_meta_val_str_by_index(
                    self.model.as_ptr(),
                    index,
                    buf_ptr,
                    buf_len,
    ...
    pub fn rope_type(&self) -> Option<RopeType> {
        match unsafe { qllama_sys::llama_model_rope_type(self.model.as_ptr()) } {
            qllama_sys::LLAMA_ROPE_TYPE_NONE => None,
            qllama_sys::LLAMA_ROPE_TYPE_NORM => Some(RopeType::Norm),
            qllama_sys::LLAMA_ROPE_TYPE_NEOX => Some(RopeType::NeoX),
            qllama_sys::LLAMA_ROPE_TYPE_MROPE => Some(RopeType::MRope),
            qllama_sys::LLAMA_ROPE_TYPE_VISION => Some(RopeType::Vision),
            rope_type => {
    ...
    pub fn chat_template(
        ...
        let result =
            unsafe { qllama_sys::llama_model_chat_template(self.model.as_ptr(), name_ptr) };
    ...
    pub fn load_from_file(
        ...
        let llama_model =
            unsafe { qllama_sys::llama_load_model_from_file(cstr.as_ptr(), params.params) };
    ...
    pub fn lora_adapter_init(
        ...
        let cstr = CString::new(path)?;
        let adapter =
            unsafe { qllama_sys::llama_adapter_lora_init(self.model.as_ptr(), cstr.as_ptr()) };
    ...
    pub fn new_context<'a>(
        ...
        let context = unsafe {
            qllama_sys::llama_new_context_with_model(self.model.as_ptr(), context_params)
        };
    ...
    pub fn apply_chat_template(
        ...
        // Build our qllama_sys chat messages
        let chat: Vec<qllama_sys::llama_chat_message> = chat
            .iter()
            .map(|c| qllama_sys::llama_chat_message {
                role: c.role.as_ptr(),
        ...
        let res = unsafe {
            qllama_sys::llama_chat_apply_template(
                tmpl_ptr,
                chat.as_ptr(),
                chat.len(),
                add_ass,
    ...
    pub fn apply_chat_template_with_tools_oaicompat(
        ...
        let chat: Vec<qllama_sys::llama_chat_message> = messages
            .iter()
            .map(|c| qllama_sys::llama_chat_message {
                role: c.role.as_ptr(),
                content: c.content.as_ptr(),
        ...
        let mut raw_result = qllama_sys::llama_rs_chat_template_result {
            prompt: ptr::null_mut(),
        ...
        let rc = unsafe {
            qllama_sys::llama_rs_apply_chat_template_with_tools_oaicompat(
                self.model.as_ptr(),
                tmpl.0.as_ptr(),
                chat.as_ptr(),
                chat.len(),
        ...
        unsafe { qllama_sys::llama_rs_chat_template_result_free(&mut raw_result) };
        result
    }
    ...
    pub fn apply_chat_template_oaicompat(
        ...
        let ffi_params = qllama_sys::llama_rs_chat_template_oaicompat_params {
        ...
        let rc = unsafe {
            qllama_sys::llama_rs_apply_chat_template_oaicompat(
                self.model.as_ptr(),
                tmpl.0.as_ptr(),
                &ffi_params,
                &mut raw_result,
...
impl ChatTemplateResult {
    ...
    pub fn parse_response_oaicompat(
        ...
        let rc = unsafe {
            qllama_sys::llama_rs_chat_parse_to_oaicompat(
                text_cstr.as_ptr(),
                is_partial,
                self.chat_format,
        ...
        unsafe { qllama_sys::llama_rs_string_free(out_json) };
        result
    }
    ...
    pub fn streaming_state_oaicompat(&self) -> Result<ChatParseStateOaicompat, ChatParseError> {
        ...
        let state = unsafe {
            qllama_sys::llama_rs_chat_parse_state_init_oaicompat(
                self.chat_format,
                self.parse_tool_calls,
                parser_cstr
                    .as_ref()
...
impl Drop for LlamaModel {
    fn drop(&mut self) {
        unsafe { qllama_sys::llama_free_model(self.model.as_ptr()) }
    }
}
...
pub enum VocabType {
    /// Byte Pair Encoding
    BPE = qllama_sys::LLAMA_VOCAB_TYPE_BPE as _,
    /// Sentence Piece Tokenizer
    SPM = qllama_sys::LLAMA_VOCAB_TYPE_SPM as _,
}
...
pub enum LlamaTokenTypeFromIntError {
    ...
    UnknownValue(qllama_sys::llama_vocab_type),
}

impl TryFrom<qllama_sys::llama_vocab_type> for VocabType {
    type Error = LlamaTokenTypeFromIntError;

    fn try_from(value: qllama_sys::llama_vocab_type) -> Result<Self, Self::Error> {
        match value {
            qllama_sys::LLAMA_VOCAB_TYPE_BPE => Ok(VocabType::BPE),
            qllama_sys::LLAMA_VOCAB_TYPE_SPM => Ok(VocabType::SPM),
