...
pub struct ChatParseStateOaicompat {
    pub(crate) state: NonNull<qllama_sys::llama_rs_chat_parse_state_oaicompat>,
}
...
impl ChatParseStateOaicompat {
    ...
    pub fn update(
        ...
        let mut out_msg: qllama_sys::llama_rs_chat_msg_oaicompat = unsafe { mem::zeroed() };
        let mut out_diffs: *mut qllama_sys::llama_rs_chat_msg_diff_oaicompat = ptr::null_mut();
        let mut out_diffs_count: usize = 0;
        let rc = unsafe {
            qllama_sys::llama_rs_chat_parse_state_update_oaicompat(
                self.state.as_ptr(),
                text_cstr.as_ptr(),
                is_partial,
                &mut out_msg,
        ...
        let result = {
            ...
            for diff in diffs {
                ...
                let rc = unsafe {
                    qllama_sys::llama_rs_chat_msg_diff_to_oaicompat_json(diff, &mut out_json)
                };
                if !status_is_ok(rc) {
                    if !out_json.is_null() {
        ...
        unsafe { qllama_sys::llama_rs_chat_msg_free_oaicompat(&mut out_msg) };
        unsafe {
            qllama_sys::llama_rs_chat_msg_diff_free_oaicompat(out_diffs, out_diffs_count)
        };
        result
    }
}
...
impl Drop for ChatParseStateOaicompat {
    fn drop(&mut self) {
        unsafe { qllama_sys::llama_rs_chat_parse_state_free_oaicompat(self.state.as_ptr()) };
    }
}
